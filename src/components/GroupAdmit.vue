    ...
    <el-container>
      ...
      <el-main>
        <div v-if="($root.USER.role==='Teacher'||$root.USER.role==='Administrator')">
          <el-table :data="admitDatas">
            <el-table-column prop="id" label="id" width="120"></el-table-column>
            <el-table-column prop="name" label="姓名"></el-table-column>
            <el-table-column prop="is_opened" label="是否授权"></el-table-column>
            <el-table-column label="操作">
              <template slot-scope="scope">
                <el-button size="mini" @click="handleEdit(scope.$index, scope.row)">改变授权</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
export default {
  ...
  methods: {
    getAdmitData(){
      this.$axios({
        method: "get",
        url: "http://localhost:5000/taskset/query_before_assign/" + this.$root.USER.id +"/" +this.$route.query.groupId,
      }).then((successResponse) => {
        this.admitDatas = successResponse.data.data.tasksets
      });
    },
    back() {
    ...
    handleEdit(index, row) {
      ...
      this.changeStyle(row.id)
      this.getAdmitData()// 方式数据刷新比修改数据快
      this.getAdmitData()
      this.getAdmitData()
      this.getAdmitData()
      this.getAdmitData()
    },
    changeStyle(tasksetId){
      this.$axios({
        method: "POST",
        url: "http://localhost:5000/taskset/assign2group/" + this.$route.query.groupId+"/" + tasksetId,
      }).then((successResponse) => {
